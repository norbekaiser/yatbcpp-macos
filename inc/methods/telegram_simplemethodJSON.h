...
namespace yatbcpp{
    ...
    public:
        ...
        telegram_simplemethodJSON(std::string functionname) : functionname(functionname){
        ...
        static bool perform_simplerequestJSON(Token T, telegram_simplemethodJSON &method_body){
            ...
            if(Response.isMember("ok")){
                curl_slist_free_all(curl_header_list);
                curl_easy_cleanup(curl);
                return Response["ok"].asBool();
            }
            else{
                curl_easy_cleanup(curl);
                curl_slist_free_all(curl_header_list);
...
#endif //YATBCPP_TELEGRAM_SIMPLEMETHODJSON_H
