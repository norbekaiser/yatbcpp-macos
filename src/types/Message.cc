...
Message Message::fromJson(Json::Value Data) {
    ...
    if(Data.isMember("audio")){
        Audio A= Audio::fromJson(Data["audio"]);
        ret.setAudio(A);
    }
    if(Data.isMember("document")){
        ret.setDocument(Document::fromJson(Data["document"]));
    }
    if(Data.isMember("photo")) {
        for (int i = 0; i < Data["photo"].size(); i++) {
            ret.addPhoto(PhotoSize::fromJson(Data["photo"][i]));
        }
    }
    //todo missing sticker
    if(Data.isMember("video")){
        ret.setVideo(Video::fromJson(Data["video"]));
    }
    if(Data.isMember("voice")){
        ret.setVoice(Voice::fromJson(Data["voice"]));
    }
    if(Data.isMember("video_note")){
        ret.setVideo_note(VideoNote::fromJson(Data["video_note"]));
    }
    if(Data.isMember("new_chat_members")) {
        for (int i = 0; i < Data["new_chat_members"].size(); i++) {
            ret.addNew_chat_members(User::fromJson(Data["new_chat_members"][i]));
        }
    }
    ...
    if(Data.isMember("contact")){
        ret.setContact(Contact::fromJson(Data["contact"]));
    }
    if(Data.isMember("location")){
        ret.setLocation(Location::fromJson(Data["location"]));
    }
    if(Data.isMember("venue")){
        ret.setVenue(Venue::fromJson(Data["venue"]));
    }
    if(Data.isMember("new_chat_member")){
        ret.setNew_chat_member(User::fromJson(Data["new_chat_member"]));
    }
    if(Data.isMember("left_chat_member")){
        ret.setLeft_chat_member(User::fromJson(Data["left_chat_member"]));
    }
    ...
    if(Data.isMember("new_chat_photo")) {
        for (int i = 0; i < Data["new_chat_photo"].size(); i++) {
            ret.addNew_chat_photo(PhotoSize::fromJson(Data["new_chat_photo"][i]));
        }
    }
